#!/usr/bin/env python3
"""Télécharge les factures Hunter.io depuis le portail.

Usage :
    python3 scripts/portals/fetch_hunter.py [--since YYYY-MM-DD] [--headed] [--init-session]
"""
...
VENDOR = "hunter"
LOGIN_URL = "https://hunter.io/sign-in"
BILLING_URL = "https://hunter.io/account/billing"


def _is_logged_in(page) -> bool:
    return "hunter.io/sign-in" not in page.url and "hunter.io/sign-up" not in page.url
...
def _login(page, email: str, password: str, init_session: bool) -> None:
    ...
    if init_session or not email:
        wait_for_manual_login(page, "Hunter.io", lambda: _is_logged_in(page))
        return
    ...
    try:
        ...
        print(f"  ⚠ Connexion Hunter.io échouée ({exc}). Relancer avec --init-session.", file=sys.stderr)
...
def _collect_invoices(page, ctx, since: str, out_dir: Path, dry_run: bool) -> list:
    ...
    for i, trigger in enumerate(download_triggers):
        ...
        filename = f"hunter_{date_iso or f'item{i:03d}'}_{amount:.2f}.pdf"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Télécharge les factures Hunter.io.")
    std_args(parser)
    args = parser.parse_args()

    email, password, _ = credentials("HUNTER")
