#!/usr/bin/env python3
"""Télécharge les factures QR Code Generator depuis le portail.

Usage :
    python3 scripts/portals/fetch_qr_gen.py [--since YYYY-MM-DD] [--headed] [--init-session]
"""
...
VENDOR = "qr_gen"
LOGIN_URL = "https://www.qr-code-generator.com/account/login/"
BILLING_URLS = [
    "https://www.qr-code-generator.com/account/billing/",
    "https://www.qr-code-generator.com/account/invoices/",
    "https://www.qr-code-generator.com/account/",
]


def _is_logged_in(page) -> bool:
    return "/login" not in page.url and "qr-code-generator.com" in page.url


def _login(page, email: str, password: str, init_session: bool) -> None:
    page.goto(LOGIN_URL, timeout=60_000)
    page.wait_for_load_state("domcontentloaded", timeout=60_000)
    if _is_logged_in(page) and "/account/" in page.url:
        return

    if init_session or not email:
        wait_for_manual_login(page, "QR Code Generator", lambda: _is_logged_in(page))
        return
    ...
    try:
        ...
        print(f"  ⚠ Connexion QR Gen échouée ({exc}). Relancer avec --init-session.", file=sys.stderr)
...
def _collect_invoices(page, ctx, since: str, out_dir: Path, dry_run: bool) -> list:
    ...
    if not loaded:
        print("  ✗ Page billing QR Gen introuvable.", file=sys.stderr)
        return []
    ...
    for i, trigger in enumerate(download_triggers):
        ...
        filename = f"qr_gen_{date_iso or f'item{i:03d}'}_{amount:.2f}.pdf"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Télécharge les factures QR Code Generator.")
    std_args(parser)
    args = parser.parse_args()

    email, password, _ = credentials("QRCODEGEN")
    sess = Path(args.session) if args.session else session_path(VENDOR)
    out_dir = Path(args.out_dir) if args.out_dir else _ROOT / "reports/portals/qr_gen/input"
