#!/usr/bin/env python3
"""Télécharge les reçus Turo depuis le portail (turo.com).

Usage :
    python3 scripts/portals/fetch_turo.py [--since YYYY-MM-DD] [--headed] [--init-session]
"""
...
VENDOR = "turo"
LOGIN_URL = "https://turo.com/us/en/signin"
TRIPS_URL = "https://turo.com/us/en/trips"


def _is_logged_in(page) -> bool:
    return "/signin" not in page.url and "/login" not in page.url and "turo.com" in page.url
...
def _login(page, email: str, password: str, init_session: bool) -> None:
    ...
    if init_session or not email:
        wait_for_manual_login(page, "Turo", lambda: _is_logged_in(page))
        page.goto(TRIPS_URL, timeout=60_000)
        return
    ...
    try:
        ...
        print(f"  ⚠ Connexion Turo échouée ({exc}). Relancer avec --init-session.", file=sys.stderr)
...
def _collect_trips(page, ctx, since: str, out_dir: Path, dry_run: bool) -> list:
    ...
    trip_cards = page.query_selector_all(
        'a[href*="/trips/"], [data-testid*="trip-card"], '
        '[class*="TripCard"], [class*="tripCard"]'
    )
    print(f"  {len(trip_cards)} location(s) trouvée(s).", file=sys.stderr)
    ...
    for i, card in enumerate(trip_cards):
        ...
        filename = f"turo_{date_iso or f'item{i:03d}'}_{amount:.2f}.pdf"
        ...
        try:
            if href:
                url = href if href.startswith("http") else "https://turo.com" + href
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Télécharge les reçus Turo.")
    std_args(parser)
    args = parser.parse_args()

    email, password, _ = credentials("TURO")
    sess = Path(args.session) if args.session else session_path(VENDOR)
    out_dir = Path(args.out_dir) if args.out_dir else _ROOT / "reports/portals/turo/input"
